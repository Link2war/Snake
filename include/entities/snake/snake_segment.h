...
{
private :
    static std::unique_ptr<Texture> m_textures[3]; // textures[BodyPart]

    Sprite m_sprite; // sprite du block
    float m_speed; // vitesse du block
    Vector2i m_direction; // direction du block
    Vector2f m_startPosition; // position initiale quand le block est sur une case
    Vector2f m_currentPosition; // position actuelle du block
    Vector2f m_targetPosition; // position prévue de la prochaine case
