...
private :
    ...
    std::vector<std::unique_ptr<Button>> m_buttons; // stocke les différents Button du Menu
    int m_currentIndex; // index du Button actuel

    float m_selectTimestamp; // timestamp du dernier select
    bool m_canNavigate; // si on peut passer d'un Button à un autre
    float m_moveTimestamp; // timestamp du dernier changement de bouton
